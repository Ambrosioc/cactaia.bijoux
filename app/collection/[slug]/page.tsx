...
import { ChevronLeft, ChevronRight, Filter, Heart, Package, Search, Star } from 'lucide-react';
...
export default function CollectionPage() {
    ...
    // Convertir le slug en nom de collection
    const getCollectionFromSlug = (slug: string) => {
        ...
        return collectionMap[slug] || slug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    };

    // Charger les produits quand les paramètres changent
    useEffect(() => {
        if (params.slug) {
            const collection = getCollectionFromSlug(params.slug as string);
            setCollectionName(collection);
            loadProducts(collection);
        }
    }, [params.slug, currentPage, sortBy, searchTerm]);

    const loadProducts = async (collection: string) => {
        try {
            setLoading(true);
            setError(null);
            ...
            <div className="pt-24 pb-12 bg-white shadow-sm">
                <div className="container-custom">
                    ...
                    <div className="text-center mb-8">
                        <h1 className="text-4xl font-bold text-gray-900 mb-4">Collection {collectionName}</h1>
                        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                            Découvrez notre collection exclusive {collectionName.toLowerCase()}
                            avec des pièces uniques et élégantes.
                        </p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20">
                        ...
                        <p className="text-gray-600 mb-8 max-w-md mx-auto">
                            {searchTerm
                                ? `Aucun produit ne correspond à "${searchTerm}" dans cette collection.`
