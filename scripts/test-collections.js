...
async function testCollections() {
    console.log('📁 Test du système de collections...\n');
    ...
    try {
        ...
        console.log(`   - ${categories?.length || 0} catégories de base`);
        console.log(`   - ${collections?.length || 0} collections de base`);

        // 2. Afficher les catégories existantes
        console.log('\n2️⃣ Catégories existantes:');
        categories?.forEach((cat, index) => {
            console.log(`   ${index + 1}. ${cat.name} (${cat.slug})`);
        });

        // 3. Afficher les collections existantes
        console.log('\n3️⃣ Collections existantes:');
        collections?.forEach((col, index) => {
            console.log(`   ${index + 1}. ${col.name} (${col.slug}) - ${col.is_featured ? '⭐' : '⚪'}`);
        });

        // 4. Tester la création d'une nouvelle collection
        console.log('\n4️⃣ Test de création d\'une collection...');
        
        const testCollection = {
            name: 'Collection Test',
            slug: 'collection-test',
            description: 'Collection de test pour vérifier le système',
            is_active: true,
            is_featured: false,
            sort_order: 100
        };
        ...
        console.log('   3. Vérifiez que les collections s\'affichent');
        console.log('   4. Testez la création d\'une nouvelle collection');
